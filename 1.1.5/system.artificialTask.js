...
var systemArtificialTask = {
    ...
    */
    run: function() {
        ...
        taskHarvest.run(2, source1, {x:19,y:16}, {x:21,y:7}, {x:21,y:8}, {x:22,y:7}, 200);

        //#3 采集能量任务2
        var source2 = Game.getObjectById('5bbcab1a9099fc012e632d31'); //能量源2
        taskHarvest.run(3, source2, {x:26,y:24}, {x:43,y:23}, {x:42,y:23}, {x:43,y:22}, 200);

        //炮台自动维修与攻击1
        var tower1 = Game.getObjectById('6551d38442580f159cfdfb5f'); //Tower1
        structureTower.run(tower1);

        //#4 资源调度任务1(矿点 -> 孵化储备、中央存储)
        ...
        taskHomeSchedul.run(4, 1, inStore1, outStore1);
        ...
        taskHomeSchedul.run(5, 1, inStore2, outStore2);

        //#6 基地建设任务(中央存储 -> 'W34N48')
        var buildStore = Game.getObjectById('6553e7654f26da05aea93b8d'); //中央存储结构Storage
        roleHomeBuilder.run(6, 0, buildStore, 'W34N48', {x:24,y:26});
