...
var queueTask_W34N48 = {
    ...
    */
    run: function() {
        ...
        var harvestStore3 = Game.getObjectById('655739435111434a7b4417d2'); //采集能量任务3 出货点
        var harvestStore4 = Game.getObjectById('6557417545e09f2211b4c614'); //采集能量任务4 出货点
        ...
        if(taskHomeSchedul.run('1-2', 2, inStore2, outStore2, moduleHomeCarrier, false, spawnName[nameRoom][0]) != 0)
        ...
        //#1-5 升级控制器任务1
        if(taskUpgrad.run('1-5', controller, upgradStore, {x:20,y:28}, spawnName[nameRoom][0]) != 0)
            reportWrongExit.run('任务#1-5 升级控制器任务1');
        //#1-6 升级控制器任务2
        if(taskUpgrad.run('1-6', controller, upgradStore, {x:19,y:29}, spawnName[nameRoom][0]) != 0)
            reportWrongExit.run('任务#1-6 升级控制器任务2');
        //#1-7 升级控制器任务3
        if(taskUpgrad.run('1-7', controller, upgradStore, {x:19,y:30}, spawnName[nameRoom][0]) != 0)
            reportWrongExit.run('任务#1-7 升级控制器任务3');
        
        //#1-8 基地建设任务1
        if(taskHomeBuilder.run('1-8', 0, centralStore, nameRoom, {x:24,y:26}, spawnName[nameRoom][0]) != 0)
        ...
        //#1-13 防御外矿1'W33N48'
        if(taskAttackInvader.run('1-13', 1, nameRoomSource, {x:40,y:31}, spawnName[nameRoom][0]) != 0)
