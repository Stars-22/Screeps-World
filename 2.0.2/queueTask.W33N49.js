...
var queueTask_W33N49 = {
    ...
    */
    run: function() {
        ...
        var outStore1 = [...spawns, ...extensions, tower1];
        if(inStore1[0].store.getUsedCapacity(RESOURCE_ENERGY) > 10000){
            //如果中央存储储量大于10000 -> 启用升级
            outStore1.push(upgradStore); //升级任务供货点
        }
        if(taskHomeSchedul.run('2-1', 1, inStore1, outStore1, moduleHomeCarrier, false, spawnName[nameRoom][0]) != 0)
        ...
        if(taskHomeSchedul.run('2-2', 1, inStore2, outStore2, moduleHomeCarrier, false, spawnName[nameRoom][0]) != 0)
        ...
        //#2-5 升级控制器任务1
        if(taskUpgrad.run('2-5', controller, upgradStore, {x:10,y:19}, spawnName[nameRoom][0]) != 0)
            reportWrongExit.run('任务#2-5 升级控制器任务1');
        //#2-6 升级控制器任务2
        if(taskUpgrad.run('2-6', controller, upgradStore, {x:10,y:20}, spawnName[nameRoom][0]) != 0)
